...
fn read_uint_v(buffer: &[u8]) -> Message {
	...
	if (buffer[0] & 1) == 1 {
        ...
        msg.message_size = ((Wrapping(buffer[2]) << 13) + (Wrapping(buffer[1]) << 5) + (Wrapping(buffer[0]) >> 3) + Wrapping(0x4080 as u16 as u8)).0 as u16;
        msg.end = 3;
	} else {
